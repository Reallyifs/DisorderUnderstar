...
{
    public class DisorderEschatologyTriangleGreen : ModProjectile
    {
            ...
            if (projectile.timeLeft <= 149)
            {
                for (int i = 0; i < 2; i++)
                {
                    Dust dust = Dust.NewDustDirect
                        ...
                        MyDustId.GreenBubble1, 0f, 0f, 100,
                        Color.Green, 1f);
                    dust.noLight = false;
                    dust.noGravity = true;
                }
        ...
        public override void Kill(int timeLeft)
        {
            for (int i = 0; i < 5; i++)
            {
                Dust dust = Dust.NewDustDirect
                    (projectile.Center, projectile.width + 2, projectile.height + 2,
                    MyDustId.GreenBubble1, 0f, 0f, 128,
                    Color.Green, 1.5f);
                dust.noLight = false;
                dust.noGravity = true;
            }
        ...
        {
            target.AddBuff(BuffID.CursedInferno, damage * 2);
